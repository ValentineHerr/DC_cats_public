...
IDSampler <- nimbleFunction(
  ...
  run = function() {
    ...
    for(i in 1:M){
      if(z[i]==1){
        for(j in 1:J){
          ll.y[i,j] <-  dnbinom(y.true[i,j],size=model$r[1]*model$K1D[j],prob=model$p[i,j], log = TRUE)
        }
      }
    }
    for(i in 1:M){
    ...
    for(l in 1:n.samples){#for all samples without known IDs
      ...
      if(denom>0){
        ...
        if(ID.cand[l]!=ID.curr[l]){
          ...
          ll.y.cand[swapped[1],this.j[l]]=dnbinom(y.true.cand[swapped[1],this.j[l]],size=model$r[1]*model$K1D[this.j[l]],prob=model$p[swapped[1],this.j[l]],log=TRUE)
          ll.y.cand[swapped[2],this.j[l]]=dnbinom(y.true.cand[swapped[2],this.j[l]],size=model$r[1]*model$K1D[this.j[l]],prob=model$p[swapped[2],this.j[l]],log=TRUE)
